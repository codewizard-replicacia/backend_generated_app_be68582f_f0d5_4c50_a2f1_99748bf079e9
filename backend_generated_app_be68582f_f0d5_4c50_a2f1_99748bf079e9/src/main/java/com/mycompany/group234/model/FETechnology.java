...
@Entity(name = "FETechnology")
@Table(name = "\"FETechnology\"", schema =  "\"generated_app\"")
@Data
                        
public class FETechnology {
	public FETechnology () {   
  }
	  
  @Id
  @GeneratedValue(strategy = GenerationType.IDENTITY)
  @Column(name = "\"FeTechId\"", nullable = true )
  ...
  private String version;
	...
	@Column(name = "\"FeTechnologyFeatures\"")
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(schema = "\"generated_app\"", name = "\"FETechnologyFeTechnologyFeatures\"",joinColumns = @JoinColumn(name = "\"FeTechId\""))
    private List<String> feTechnologyFeatures = new ArrayList<>();
...
@JoinTable(
            name="\"FETechnologySupported_Capabilities\"",
            joinColumns = @JoinColumn( name="\"FeTechId\""),
...
@OneToMany(fetch = FetchType.LAZY, cascade = {CascadeType.PERSIST, CascadeType.REMOVE, CascadeType.MERGE})
@JoinTable(
            name="\"FETechnologySupported_BETechnologies\"",
            joinColumns = @JoinColumn( name="\"FeTechId\""),
            inverseJoinColumns = @JoinColumn( name="\"BeTechId\""), schema = "\"generated_app\"")
private List<BETechnology> supported_BETechnologies = new ArrayList<>();

@OneToMany(fetch = FetchType.LAZY, cascade = {CascadeType.PERSIST, CascadeType.REMOVE, CascadeType.MERGE})
@JoinTable(
            name="\"FETechnologyScreens\"",
            joinColumns = @JoinColumn( name="\"FeTechId\""),
            inverseJoinColumns = @JoinColumn( name="\"FeScreenId\""), schema = "\"generated_app\"")
private List<FrontendScreen> screens = new ArrayList<>();
  ...
  + "Icon= " + icon  + ", " 
  + "Version= " + version  + ", " 
  + "FeTechnologyFeatures= " + feTechnologyFeatures 
 + "]";
	}
